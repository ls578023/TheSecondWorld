        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public override int Capacity
            {
                ...
                set
                {
                    ...
                    //Util.LogForMat("Object pool '{0}' capacity changed from '{1}' to '{2}'.", Utility.Text.GetFullName<T>(Name), m_Capacity.ToString(), value.ToString());
                    m_Capacity = value;
                    Release();
                }
            }
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    ...
                    //Util.LogForMat("Object pool '{0}' expire time changed from '{1}' to '{2}'.", Utility.Text.GetFullName<T>(Name), m_ExpireTime.ToString(), value.ToString());
                    m_ExpireTime = value;
                    Release();
                }
            }
            ...
            public void Register(T obj, bool spawned)
            {
                ...
                //Util.LogForMat(spawned ? "Object pool '{0}' create and spawned '{1}'." : "Object pool '{0}' create '{1}'.", Utility.Text.GetFullName<T>(Name), obj.Name);
            ...
            public T Spawn(string name)
            {
                foreach (Object<T> obj in m_Objects)
                {
                    ...
                    if (m_AllowMultiSpawn || !obj.IsInUse)
                    {
                        Utils.LogForMat("Object pool '{0}' spawn '{1}'.", Utility.Text.GetFullName<T>(Name), obj.Peek().Name);
                        return obj.Spawn();
                    }
                }
            ...
            public void Unspawn(T obj)
            {
                ...
                Unspawn(obj.HashCode);
            }


            public void Unspawn(int hashCode)
            {
                if (hashCode == 0)
                {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    if (obj.Peek().HashCode == hashCode)
                    {
                        Utils.LogForMat("Object pool '{0}' unspawn '{1}'.", Utility.Text.GetFullName<T>(Name), obj.Peek().Name);
                        obj.Unspawn();
                        Release();
                        return;
                    }
            ...
            public void SetLocked(object target, bool locked)
            {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    if (obj.Peek().Target == target)
                    {
                        Utils.LogForMat("Object pool '{0}' set locked '{1}' to '{2}.", Utility.Text.GetFullName<T>(Name), obj.Peek().Name, locked.ToString());
                        obj.Locked = locked;
                        return;
                    }
                }
            ...
            public void SetPriority(object target, int priority)
            {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    if (obj.Peek().Target == target)
                    {
                        Utils.LogForMat("Object pool '{0}' set priority '{1}' to '{2}.", Utility.Text.GetFullName<T>(Name), obj.Peek().Name, priority.ToString());
            ...
            public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
            {
                ...
                foreach (ObjectBase toReleaseObject in toReleaseObjects)
                {
                    ...
                    foreach (Object<T> obj in m_Objects)
                    {
                        ...
                        obj.Release();
                        Utils.LogForMat("Object pool '{0}' release '{1}'.", Utility.Text.GetFullName<T>(Name), toReleaseObject.Name);
                        found = true;
                        break;
                    }
            ...
            public override void ReleaseAllUnused()
            {
                ...
                while (current != null)
                {
                    ...
                    Utils.LogForMat("Object pool '{0}' release '{1}'.", Utility.Text.GetFullName<T>(Name), current.Value.Name);
                    current = next;
                }
            }
            ...
            internal override void Update(float elapseSeconds, float realElapseSeconds)
            {
                ...
                Utils.LogForMat("Object pool '{0}' auto release start.", Utility.Text.GetFullName<T>(Name));
                Release();
                Utils.LogForMat("Object pool '{0}' auto release complete.", Utility.Text.GetFullName<T>(Name));
            }
