...
{
    public class AudioFadeAction : AudioActionBase
    {
        //最大音量，配置来的std_audio
        ...
        public AudioFadeAction(GameObject root, int audioId, Action<int> startCall, Action<int> endCall) : base(root, audioId, startCall, endCall)
        {
        }
        ...
        protected override async Task<AudioSource> OnPlay(AudioData audioConfig)
        {
            if (audioConfig != null)
            {
                ...
                var name = "fade_" + this.GetAudioGuid();
        ...
        protected override void OnUpdate()
        {
            if (base.s_audioSource != null)
            {
                if (this.m_EndTime <= 0)
                {
                    ...
                    //if (isPlaying == false && !this.m_IsFadeEnding)
                    //{
                    //    base.Stop();
                    //}
