...
public class Reporter : MonoBehaviour
{
    ...
    bool isGestureDone_Click() 
    {
        //每一秒清空点击次数
        if (Time.time>MouseClickTimer)
        {
            MouseClickCount = 0;
            MouseClickTimer = Time.time + 1;
            //Debug.Log("清空");
        }
        ...
        {
            MouseClickCount++;
            Debug.Log("MouseClickCount：" + MouseClickCount);
        }
    ...
    /// 点击触发日志信息
    /// </summary>
    public bool GestureDoneByClick;
    ...
    void Update()
	{
		...
		calculateStartIndex();
		if (!show && GestureDoneByClick?isGestureDone_Click():isGestureDone()) {
			doShow();
		}
